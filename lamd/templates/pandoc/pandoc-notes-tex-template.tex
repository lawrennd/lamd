...
$if(logo)$
\rhead{\includegraphics[width=2cm]{$logo$}\\$if(employee)$$employee.given$ $employee.family$ Launch Plan$if(employee.role)$\\$employee.role$$endif$$if(employee.org)$, $employee.org$$endif$$endif$}
$endif$
...
$if(author)$
\author{$for(author)$$author.given$ $author.family$$sep$\\$endfor$}
$endif$
