...
class LM(ProbMapModel):
    ...
    def log_likelihood(self):
        ...
        self.update_sum_squares()
        return (
            -self.num_data / 2.0 * log(pi * 2.0)
            - self.num_data / 2.0 * log(self.sigma2)
            - self.sum_squares / (2.0 * self.sigma2)
        )
...
def polynomial(x, num_basis=4, data_limits=[-1.0, 1.0]):
    "Polynomial basis"
...
def radial(x, num_basis=4, data_limits=[-1.0, 1.0], width=None):
    ...
    if num_basis > 1:
        ...
        centres = asarray([data_limits[0] / 2.0 + data_limits[1] / 2.0])
        if width is None:
...
def fourier(x, num_basis=4, data_limits=[-1.0, 1.0], frequency_range=None):
    """Fourier basis"""
    tau = 2 * pi
...
def relu(x, num_basis=4, data_limits=[-1.0, 1.0], gain=None):
    ...
    Phi[:, 0] = 1.0
    for i in range(1, num_basis):
        Phi[:, i : i + 1] = (gain[i - 1] * asarray(x, dtype=float) > centres[i - 1]) * (
            asarray(x, dtype=float) - centres[i - 1]
        )
    return Phi


def tanh(x, num_basis=4, data_limits=[-1.0, 1.0], gain=None):
    """Hyperbolic tangents"""
    if num_basis > 2:
        centres = linspace(data_limits[0], data_limits[1], num_basis - 1)
        width = (centres[1] - centres[0]) / 2.0
...
class NonparametricDropoutNeuralNetwork(SimpleDropoutNeuralNetwork):
    ...
    def __init__(self, alpha=10, beta=1, n=1000):
        ...
        tot = log(n) + self.gamma + 0.5 / n * (1.0 / 12.0) / (n * n)
...
class BLM(LM):
    ...
    def update_nll(self):
        ...
        self.log_det = self.num_data * log(self.sigma2 * pi * 2.0) - 2 * log(abs(det(self.Q[self.y.shape[0] :, :])))
...
def load_pgm(filename, directory=None, byteorder=">"):
    ...
    return frombuffer(
        buffer, dtype="u1" if int(maxval) < 256 else byteorder + "u2", count=int(width) * int(height), offset=len(header)
...
def exponentiated_quadratic(x, x_prime, variance=1.0, lengthscale=1.0):
    """Exponentiated quadratic covariance function."""
    r = norm(x - x_prime, 2)
    return variance * exp((-0.5 * r * r) / lengthscale**2)


def eq_cov(x, x_prime, variance=1.0, lengthscale=1.0):
    """Exponentiated quadratic covariance function."""
    diffx = x - x_prime
    return variance * exp(-0.5 * dot(diffx, diffx) / lengthscale**2)


def ou_cov(x, x_prime, variance=1.0, lengthscale=1.0):
    """Exponential covariance function."""
    diffx = x - x_prime
    return variance * exp(-sqrt(dot(diffx, diffx)) / lengthscale)


def matern32_cov(x, x_prime, variance=1.0, lengthscale=1.0):
...
def matern52_cov(x, x_prime, variance=1.0, lengthscale=1.0):
...
def mlp_cov(x, x_prime, variance=1.0, w=1.0, b=5.0, alpha=1.0):
    ...
    theta = arccos(arg)
    return variance * 0.5 * (1.0 - theta / pi)
...
def relu_cov(x, x_prime, variance=1.0, scale=1.0, w=1.0, b=5.0, alpha=0.0):
    ...
    theta = arccos(arg)
    return (
        variance * 0.5 * ((1.0 - theta / pi) * inner + h(arg2, inner_2, s, alpha) / pi + h(arg2, inner_1, s_prime, alpha) / pi)
    )


def polynomial_cov(x, x_prime, variance=1.0, degree=2.0, w=1.0, b=1.0):
    """Polynomial covariance function."""
    return variance * (dot(x, x_prime) * w + b) ** degree


def linear_cov(x, x_prime, variance=1.0):
    """Linear covariance function."""
    return variance * dot(x, x_prime)


def bias_cov(x, x_prime, variance=1.0):
    """Bias covariance function."""
    return variance


def mlp_cov(x, x_prime, variance=1.0, w=1.0, b=1.0):
    """MLP covariance function."""
    return variance * arcsin((w * dot(x, x_prime) + b) / sqrt((dot(x, x) * w + b + 1) * (dot(x_prime, x_prime) * w + b + 1)))


def sinc_cov(x, x_prime, variance=1.0, w=1.0):
...
def brownian_cov(t, t_prime, variance=1.0):
...
def periodic_cov(x, x_prime, variance=1.0, lengthscale=1.0, w=1.0):
    """Periodic covariance function"""
    r = norm(x - x_prime, 2)
    return variance * exp(-2.0 / (lengthscale * lengthscale) * sin(pi * r * w) ** 2)


def ratquad_cov(x, x_prime, variance=1.0, lengthscale=1.0, alpha=1.0):
    """Rational quadratic covariance function"""
    r = norm(x - x_prime, 2)
    return variance * (1.0 + r * r / (2 * alpha * lengthscale * lengthscale)) ** -alpha
