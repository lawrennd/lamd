...
if not MDFIELD_SERVER_SCRIPT.exists():
    raise FileNotFoundError(
        f"mdfield-server script not found at {MDFIELD_SERVER_SCRIPT}. " "Please run deployment steps first."
    )
...
class TestConfigFallback:
    ...
    @pytest.mark.skip(
        reason="Known limitation: server config fallback doesn't work with relative paths in different directories"
    )
...
""")
        ...
        shell_result = subprocess.run(
            [str(MDFIELD_SERVER_SCRIPT), "default_field", str(markdown_file)], capture_output=True, text=True, cwd=tmp_path
        )
...
class TestErrorHandling:
    ...
    def test_missing_markdown_file(self, tmp_path):
        ...
        result = subprocess.run(
            [str(MDFIELD_SERVER_SCRIPT), "title", "nonexistent.md"], capture_output=True, text=True, cwd=tmp_path
        )
    ...
    def test_empty_markdown_file(self, tmp_path):
        ...
        result = subprocess.run(
            [str(MDFIELD_SERVER_SCRIPT), "title", str(empty_file)], capture_output=True, text=True, cwd=tmp_path
        )
...
class TestPerformance:
    ...
    def test_speedup(self, test_markdown_file):
        ...
        for field in fields:
            subprocess.run(
                ["mdfield", "--no-server", field, str(test_markdown_file)], capture_output=True, cwd=test_markdown_file.parent
            )
    ...
    def test_server_persistence(self, test_markdown_file):
        ...
        subprocess.run(
            [str(MDFIELD_SERVER_SCRIPT), "title", str(test_markdown_file)], capture_output=True, cwd=test_markdown_file.parent
        )

        # Second call should be very fast (server already running)
        start = time.perf_counter()
        subprocess.run(
            [str(MDFIELD_SERVER_SCRIPT), "author", str(test_markdown_file)], capture_output=True, cwd=test_markdown_file.parent
        )
...
class TestRealWorldFiles:
    ...
    def test_with_actual_talk_file(self):
        ...
        for field in ["title", "author", "date"]:
            python_result = subprocess.run(
                ["mdfield", "--no-server", field, str(talk_file)], capture_output=True, text=True, cwd=talk_file.parent
            )

            shell_result = subprocess.run(
                [str(MDFIELD_SERVER_SCRIPT), field, str(talk_file)], capture_output=True, text=True, cwd=talk_file.parent
            )

            # Normalize outputs (handles JSON formatting differences)
            assert normalize_output(python_result.stdout) == normalize_output(
                shell_result.stdout
