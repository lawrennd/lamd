...
def get_git_status() -> Dict[str, Any]:
    ...
    for line in status_output.split("\n"):
        ...
        if status.startswith("??"):
            ...
            git_info["modified_files"].append({"status": status.strip(), "path": file_path})
    ...
    for cip_file in sorted(glob.glob("cip/cip*.md")):
        ...
        if frontmatter:
            ...
            if status == "proposed":
                ...
                cips_info["by_status"]["accepted"].append({"id": file_id, "title": frontmatter.get("title", "Untitled")})
            elif status == "implemented":
                cips_info["by_status"]["implemented"].append({"id": file_id, "title": frontmatter.get("title", "Untitled")})
            elif status == "closed":
                cips_info["by_status"]["closed"].append({"id": file_id, "title": frontmatter.get("title", "Untitled")})
        else:
            ...
            cips_info["without_frontmatter"].append({"id": file_id, "title": title, "path": cip_file})

            # Add to status lists even without frontmatter
            if status == "proposed":
                cips_info["by_status"]["proposed"].append({"id": file_id, "title": title, "no_frontmatter": True})
            elif status == "accepted":
                cips_info["by_status"]["accepted"].append({"id": file_id, "title": title, "no_frontmatter": True})
            elif status == "implemented":
                cips_info["by_status"]["implemented"].append({"id": file_id, "title": title, "no_frontmatter": True})
            elif status == "closed":
                cips_info["by_status"]["closed"].append({"id": file_id, "title": title, "no_frontmatter": True})
...
def scan_backlog() -> Dict[str, Any]:
    ...
    backlog_info = {
        ...
        "by_status": {"proposed": [], "ready": [], "in_progress": [], "completed": [], "abandoned": []},
    }
    ...
    backlog_dirs = ["backlog/bugs/", "backlog/features/", "backlog/documentation/", "backlog/infrastructure/"]
    ...
    for directory in backlog_dirs:
        ...
        for backlog_file in sorted(glob.glob(f"{directory}/*.md")):
            ...
            if frontmatter:
                ...
                item_info = {"id": file_id, "title": frontmatter.get("title", "Untitled"), "path": backlog_file}
                ...
                item_info = {"id": file_id, "title": title, "path": backlog_file, "no_frontmatter": True}
...
def generate_next_steps(
    git_info: Dict[str, Any], cips_info: Dict[str, Any], backlog_info: Dict[str, Any], requirements_info: Dict[str, Any]
) -> List[str]:
    ...
    # Add suggestion if no patterns exist
        ...
        next_steps.append("Create requirements patterns in ai-requirements/patterns/")
    # Add suggestion if no prompts exist in any category
    elif all(not prompts for prompts in requirements_info["prompts"].values()):
        next_steps.append("Create requirements prompts in ai-requirements/prompts/")
    ...
    if requirements_info["has_framework"] and backlog_info["by_status"]["proposed"]:
        next_steps.append("Use AI-Requirements patterns to refine proposed backlog tasks")
    ...
    if requirements_info["has_framework"]:
        ...
        if requirements_related_items:
            next_steps.append(
                f"Continue implementation of requirements-related item: {requirements_related_items[0]['title']}"
            )
    ...
    if cips_info and cips_info.get("by_status") and cips_info["by_status"].get("accepted"):
        next_steps.append(
            f"Implement accepted CIP: {cips_info['by_status']['accepted'][0]['id']} - {cips_info['by_status']['accepted'][0]['title']}"
        )
    ...
    if not next_steps:
        ...
        if requirements_info["has_framework"]:
            ...
            if any("goal-decomposition" in pattern for pattern in requirements_info["patterns"]):
                next_steps.append(
                    "Use the Goal Decomposition Pattern to break down high-level goals into specific requirements"
                )
            if any("stakeholder-identification" in pattern for pattern in requirements_info["patterns"]):
                next_steps.append(
                    "Use the Stakeholder Identification Pattern to identify all stakeholders for upcoming features"
                )
...
def main():
    ...
    if args.cip_only:
        ...
        next_steps = generate_next_steps(
            git_info,
            {"by_status": {"proposed": []}},
            {"by_status": {"proposed": []}, "by_priority": {"high": []}},
            requirements_info,
        )
    else:
