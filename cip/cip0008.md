...
- [ ] Phase 1b: lynguine API Enhancements (prerequisite for Phase 2)
  ...
  - Need API to extract markdown frontmatter fields via server
- [ ] Phase 2: Full Integration
  - Implement server mode field extraction in mdfield
  - Implement server mode data loading in mdlist
  - Add session caching by config file path
  - Comprehensive testing
...
### 2026-01-03 - Phase 1 Infrastructure Complete
...
**Status**: Infrastructure in place, but full functionality requires lynguine API enhancements.

**Next Steps**:
1. **lynguine work needed**: Add server endpoints for:
   - Interface field extraction (for mdfield)
   - talk_field functionality (for markdown frontmatter)
   - CustomDataFrame operations used by mdlist (already in Phase 5)
2. **lamd work**: Implement server mode extraction once lynguine APIs available
3. **Testing**: Validate performance improvements with real CV builds

**Current limitation**: mdfield and mdlist have server mode flags but currently fall back to direct mode with a note. This is intentional - the infrastructure is ready, we just need the lynguine server APIs.
