...
- `mdfield-server.sh` - Shell-based client implementation
...
1. Comprehensive test suite (unit, integration, error handling)
2. Tests must verify identical output to Python mdfield
3. Error handling tests (missing deps, server failures, etc.)
4. Integration into lamd repository and Makefiles
...
**File**: `mdfield-server.sh` (currently in `~/lawrennd/talks/_ai/` as test artifact)

**Installation**:
```bash
# 1. Move to lamd scripts directory
cp ~/lawrennd/talks/_ai/mdfield-server.sh /Users/neil/lawrennd/lamd/lamd/scripts/
chmod +x /Users/neil/lawrennd/lamd/lamd/scripts/mdfield-server.sh

# 2. Update setup.py to install it
# Add to scripts=['lamd/scripts/mdfield-server']

# 3. Or symlink for immediate use:
ln -s ~/lawrennd/talks/_ai/mdfield-server.sh ~/bin/mdfield-server
```

**Makefile Integration** (Option A - Easiest):
```make
# At top of Makefile, add alias:
MDFIELD = mdfield-server

# All existing calls automatically use shell client:
TITLE = $(shell $(MDFIELD) title $(TALK_FILE))
AUTHOR = $(shell $(MDFIELD) author $(TALK_FILE))
```

**Makefile Integration** (Option B - Explicit):
```make
# Replace individual calls:
TITLE = $(shell mdfield-server title $(TALK_FILE))
```
